...
function DashboardUserButton({ user }) {
    ...
    const roleLabel = user?.is_admin ? 'Admin' : user?.is_seller ? 'Seller' : 'Member';
    const roleColor = user?.is_admin
        ? 'bg-violet-100 text-violet-700'
        : user?.is_seller
            ? 'bg-amber-100 text-amber-700'
            : 'bg-emerald-100 text-emerald-700';
    const avatarColor = user?.is_admin ? 'bg-violet-600' : 'bg-brand';
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            <button
                ...
                className={`
                    group flex items-center gap-2.5 rounded-xl border px-2.5 py-1.5
                    transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand/50
                    ${open
                        ? 'border-brand/25 bg-brand/5 shadow-sm'
                        : 'border-gray-200 bg-white hover:border-brand/25 hover:bg-gray-50 hover:shadow-sm'
                    }
                `}
            >
                {/* Avatar + status dot */}
                <div className="relative flex-shrink-0">
                    <div className={`flex h-7 w-7 items-center justify-center rounded-full text-[11px] font-bold text-white transition-transform duration-150 group-hover:scale-105 ${avatarColor}`}>
                ...
                <div className="hidden sm:flex flex-col items-start leading-none">
                    <span className="max-w-[80px] truncate text-[12px] font-semibold text-gray-800">
                        {firstName}
                    </span>
                    <span className={`mt-0.5 rounded-full px-1.5 py-px text-[9px] font-bold uppercase tracking-wide ${roleColor}`}>
                        {roleLabel}
                    </span>
                </div>
                ...
                {/* Chevron */}
                <svg
                    className={`hidden sm:block h-3 w-3 flex-shrink-0 text-gray-400 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
            ...
            {/* ── Dropdown ── */}
            {open && (
                <div className="absolute right-0 top-full z-50 mt-2 w-64 origin-top-right" role="menu">
                    {/* Pointer triangle */}
                    ...
                    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl shadow-gray-200/60">
                        ...
                        <div className="flex items-center gap-3 border-b border-gray-100 bg-gray-50/70 px-4 py-3.5">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <span className={`mt-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${roleColor}`}>
                                    {roleLabel}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="py-1.5" role="none">
                            <Link
                                href={user?.is_admin ? route('admin.profile') : route('profile.edit')}
                                role="menuitem"
                                onClick={() => setOpen(false)}
                                className="flex items-center gap-3 px-4 py-2.5 text-[13px] font-medium text-gray-700 transition-colors hover:bg-gray-50 hover:text-gray-900 focus:outline-none"
                            >
                            ...
                            {user?.is_admin && (
                                <Link
                                    ...
                                    role="menuitem"
                                    onClick={() => setOpen(false)}
                                    className="flex items-center gap-3 px-4 py-2.5 text-[13px] font-medium text-violet-700 transition-colors hover:bg-violet-50 focus:outline-none"
                                >
                                    ...
                                    Admin Panel
                                    <span className="ml-auto rounded-full bg-violet-100 px-1.5 py-0.5 text-[10px] font-bold text-violet-700">Admin</span>
                                </Link>
                            )}
                        </div>
                        ...
                        <div className="border-t border-gray-100 py-1.5" role="none">
                            <button
                                ...
                                className="flex w-full items-center gap-3 px-4 py-2.5 text-[13px] font-medium text-red-600 transition-colors hover:bg-red-50 focus:outline-none"
                            >
