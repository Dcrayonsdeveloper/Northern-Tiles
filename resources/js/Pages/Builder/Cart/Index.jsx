...
/**
 * Trade cart. Same behaviour as the retail cart, but every endpoint is the
 * builder-scoped one and the layout / links stay inside the trade portal.
 *
 *   - api  →  /api/builder/cart/*   (BuilderCartController extends the retail one, forcing channel='trade')
 *   - checkout link → /builder/checkout
 *   - cart-updated event is `cart-updated:trade` so the retail header badge
 *     never bumps from a trade action and vice versa.
 *   - Continue-shopping and empty-cart links point at /builder/shop.
 */
...
function CartCrossSell({ products, onAdd, disabled }) {
    ...
    return (
        <div className="mt-8">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">You May Also Like (Trade Pricing)</h2>
            <div ref={ref} className="flex gap-4 overflow-x-auto scroll-smooth pb-2" style={{ scrollbarWidth: 'none' }}>
                {products.map(p => (
                    <div key={p.id} className="flex-shrink-0 w-[180px] rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
                        <Link href={route('builder.products.show', p.slug)}>
...
export default function Index({ items: initialItems, subtotal: initialSubtotal, crossSellProducts }) {
    ...
    const fetchCart = useCallback(async () => {
        try {
            ...
            console.error('Failed to fetch trade cart:', error);
        }
    }, []);
    ...
    const updateQty = async (itemId, newQty) => {
        ...
        try {
            const response = await fetch(`/api/builder/cart/${itemId}`, {
                method: 'PUT',
                headers: {
    ...
    const remove = async (itemId) => {
        ...
        try {
            ...
            console.error('Failed to remove trade item:', error);
            setItems(prevItems);
            setSubtotal(prevSubtotal);
        } finally {
            setUpdating(null);
    ...
    const addCrossSell = async (productId) => {
        try {
            await fetch('/api/builder/cart/add', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
            ...
            console.error('Failed to add trade item:', e);
        }
    };

    return (
        <BuilderLayout title="Trade Cart">
            <Head title="Trade Cart" />
            <Container>
                {isInactive && (
                ...
                <div className="flex items-end justify-between pt-6">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">Trade Cart</h1>
                        <p className="mt-1 text-sm text-gray-600">Trade pricing applied. Review before checkout.</p>
                    </div>
                    <Link
                ...
                {items.length === 0 ? (
                    <div className="mt-8 rounded-lg border bg-white p-8 text-center shadow-sm">
                        <div className="text-sm text-gray-700">Your trade cart is empty.</div>
                        <Link
                            href={route('builder.shop.index')}
                    ...
                    <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <Link
                                                        href={route('builder.products.show', item.product?.slug)}
                                                        className="text-sm font-semibold text-gray-900 hover:underline"
                                                    >
                                                        {item.product?.name}
                                                    </Link>
                                                    <div className="mt-1 text-sm text-gray-600">
                                                        ${parseFloat(item.price || 0).toFixed(2)} / sqm <span className="text-amber-600 font-semibold">Trade</span>
                                                    </div>
                                                </div>
                        ...
                        <div className="rounded-lg border bg-white p-4 shadow-sm">
                            <div className="text-sm font-semibold text-gray-900">Trade Summary</div>
                            ...
                            {isInactive ? (
                                ...
                                <Link
                                    href={route('builder.checkout.index')}
                                    className="mt-4 block rounded-md bg-amber-500 px-4 py-2 text-center text-sm font-semibold text-slate-900 hover:bg-amber-400"
                                >
                                    Trade Checkout
                                </Link>
                            )}
