...
function SectionLibrary({ registry, onAdd, onClose }) {
    ...
    const categories = ['all', ...new Set(registry.map(s => s.category).filter(Boolean))];

    const filteredSections = registry.filter(section => {
...
function SectionItem({ section, index, total, registry, onUpdate, onMove, onRemove, onToggle }) {
    ...
    const registryItem = registry.find(r => r.section_key === section.section_key);
