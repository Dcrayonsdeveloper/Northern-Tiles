...
class ImportDoc11Command extends Command
{
    protected $signature = 'ntd:import-doc11
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 4 Baltic Stone porcelain products from products_doc11.json (Beige, Chalk, Dark Grey, Grigio).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc11.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc11.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc11...");
        $this->newLine();
