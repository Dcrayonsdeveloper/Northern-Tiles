...
class ImportDoc15Command extends Command
{
    protected $signature = 'ntd:import-doc15
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 21 20mm & 10mm External Porcelain products from products_doc15.json.';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc15.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc15.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc15...");
        $this->newLine();
