...
class ImportDoc9Command extends Command
{
    protected $signature = 'ntd:import-doc9
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 5 Engineered Oak products from products_doc9.json (ARIA, DOTA, Natural Oak, Natural Oak Herringbone, Ordel Opal Oak).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc9.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc9.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc9...");
        $this->newLine();
