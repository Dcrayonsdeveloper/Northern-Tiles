...
class ImportDoc5Command extends Command
{
    protected $signature = 'ntd:import-doc5
                            {--sku=     : Only process a single SKU}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Update 10 products with SEO content from products_doc5.json (Genesis Cocoa Beige + Milano series).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc5.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc5.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc5...");
        $this->newLine();
