...
class ImportDoc8Command extends Command
{
    protected $signature = 'ntd:import-doc8
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 6 Herringbone products from products_doc8.json (Blackbutt, Classic Oak, Do Smoked, Grey Ghost, Hatton Oak, Spotted Gum).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc8.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc8.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc8...");
        $this->newLine();
