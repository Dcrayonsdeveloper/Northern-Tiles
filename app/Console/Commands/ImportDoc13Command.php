...
class ImportDoc13Command extends Command
{
    protected $signature = 'ntd:import-doc13
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 8 terrazzo porcelain products from products_doc13.json (Quartz, Sparkle, Grey, Light Grey, Venetian Marble series).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc13.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc13.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc13...");
        $this->newLine();
