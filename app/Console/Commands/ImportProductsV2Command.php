...
class ImportProductsV2Command extends Command
{
    protected $signature = 'ntd:import-v2
                            {--sku=     : Only update a single SKU}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Update 100 products with SEO content from products_v2.json (Google Doc batch).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_v2.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_v2.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products...");
        $this->newLine();
        ...
        foreach ($docs as $sku => $doc) {
            ...
            if (!$product) {
                $this->line("  <error>MISSING</error> {$sku}");
                $this->stats['missing']++;
                continue;
            }
            ...
            // Name: update if doc name is longer (more descriptive)
            $docName = trim($doc['name'] ?? '');
            if ($docName && (empty($product->name) || strlen($docName) > strlen($product->name) || $force)) {
                $updates['name'] = $docName;
                $actions[] = 'name';
            }

            // Slug: always apply computed slug (with uniqueness check)
            $docSlug = trim($doc['slug'] ?? '');
            if ($docSlug && ($product->slug !== $docSlug || $force)) {
                $uniqueSlug = $this->resolveSlug($docSlug, $product->id);
            ...
            if ($metaTitle && (empty($product->meta_title) || $product->meta_title === $product->name || $force)) {
