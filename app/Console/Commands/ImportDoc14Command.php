...
class ImportDoc14Command extends Command
{
    protected $signature = 'ntd:import-doc14
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 10 Italian Porcelain products from products_doc14.json (Concrete, Stone Focus, X-Rock series).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc14.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc14.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc14...");
        $this->newLine();
