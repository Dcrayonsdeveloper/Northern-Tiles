...
class ImportDoc7Command extends Command
{
    protected $signature = 'ntd:import-doc7
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 4 Hybrid Tile products from products_doc7.json (Moon Grey, Terrazzo Grey, Carrara, Concrete Grey).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc7.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc7.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc7...");
        $this->newLine();
