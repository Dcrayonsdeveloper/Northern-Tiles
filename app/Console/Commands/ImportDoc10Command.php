...
class ImportDoc10Command extends Command
{
    protected $signature = 'ntd:import-doc10
                            {--sku=     : Only process a single slug}
                            {--dry-run  : Preview changes without saving}
                            {--force    : Overwrite fields even if already set}';

    protected $description = 'Import/update 4 SWARD porcelain tile products from products_doc10.json (Dark Grey, Grey, Sand, White).';
    ...
    public function handle(): int
    {
        ...
        $jsonPath = storage_path('app/products_doc10.json');
        ...
        if (!$docs) {
            $this->error('Failed to parse products_doc10.json');
            return self::FAILURE;
        }

        $mode = $dryRun ? '[DRY RUN] ' : '';
        $this->info("{$mode}Processing " . count($docs) . " products from doc10...");
        $this->newLine();
