...
class PageController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        // Get trashed pages for trash tab
        ...
        return Inertia::render('Admin/Pages/Index', [
            ...
            'filters' => $request->only(['status', 'search', 'parent_id', 'trashed']),
    ...
    {
        $sectionRegistry = $this->pageService->getAvailableSections();

        return Inertia::render('Admin/Pages/Create', [
            'page' => null,
            'authors' => Author::active()->orderBy('name')->get(['id', 'name']),
            'templates' => $this->getTemplates(),
            'parentPages' => $this->pageService->getPagesForParentSelect(),
            'statuses' => Page::STATUSES,
    ...
    public function edit(Page $page): Response
    {
        ...
        return Inertia::render('Admin/Pages/Edit', [
            ...
            'authors' => Author::active()->orderBy('name')->get(['id', 'name']),
            'templates' => $this->getTemplates(),
            'parentPages' => $this->pageService->getPagesForParentSelect($page->id),
            'statuses' => Page::STATUSES,
            'sectionRegistry' => $sectionRegistry,
        ]);
    }
    ...
    public function bulkDelete(Request $request): RedirectResponse
    {
        ...
        // Check if any page has children
        $pagesWithChildren = Page::whereIn('id', $request->ids)
            ->whereHas('children')
            ->count();

        if ($pagesWithChildren > 0) {
            return back()->with('error', 'Cannot delete pages that have child pages. Move or delete children first.');
        }
        ...
        return redirect()
            ...
            ->with('success', count($request->ids) . ' pages moved to trash.');
    }
    ...
    public function bulkStatus(Request $request): RedirectResponse
    {
        ...
        return redirect()
            ->route('admin.pages.index')
            ->with('success', count($request->ids) . ' pages updated.');
    }
