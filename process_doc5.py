"""
Parse all products from doc5 (1Q0ePOt_gl8r3wu7P98AdqHn4vD32mietInOLVDYI6GI).
Output: storage/app/products_doc5.json

Doc5 format:
  # (NTD4036) PRODUCT NAME   <- SKU delimiter
  # **PRODUCT NAME**          <- product title (H1 bold, skip from description)
  ## **Meta Description:**    <- H2 meta
  ## **Product Overview**     <- H2 overview -> <p> intro
  ## **Product Specifications**  <- H2 specs
  ### **Section Name**        <- H3 sections -> <h3> in output
"""
import json, re

RAW_FILE = r"c:\Users\pc\Desktop\Northern-Tiles\storage\app\doc5_raw.txt"
OUT_FILE = r"c:\Users\pc\Desktop\Northern-Tiles\storage\app\products_doc5.json"
...
# Split on "# (NTDxxxx)" lines
parts = re.split(r'(?m)^#\s+\(NTD(\d+)\)[^\n]*$', text)
bodies = {}
i = 1
while i < len(parts) - 1:
    sku = 'NTD' + parts[i].strip()
    bodies[sku] = parts[i + 1].strip()
    i += 2

print(f"Found {len(bodies)} product blocks")

# ─── Category (all floor-wall-tiles for this doc) ────────────────────────────
...
for sku, p in results.items():
    ...
    sl = p['slug'][:60]
    d  = len(p['description'])
    sc = len(p['specifications'])
    m  = bool(p['meta_description'])
    print(f"{sku}: {nm!r} | slug={sl} | desc={d}ch | specs={sc} | meta={'Y' if m else 'N'}")
